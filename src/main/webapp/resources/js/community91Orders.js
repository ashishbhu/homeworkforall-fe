$(document).ready(
		function() {
			...
			var statisticsUrl = SITEBASEURL + "community91OpsOrder/order-count";
...
console.log(result);
	...
	$("#communityPlacedOrder").html("Total : "+ result.communityPlacedOrder);
	$("#communityPlacedOrderToday").html("Today : "+ 	result.communityPlacedOrderToday);
	...
	if (result.orderDelivered == 0) {
		...
		$("#codOrderCreatedToday").html("Community91 Orders");
	}
...
var reportUrl = SITEBASEURL + "community91OpsOrder/pagelist?orderState="
								...
								function() {
									...
									$(this).unbind().on(
											...
											function() {
												...
												+ 'community91OpsOrder/get-order-by-state-seller/'
												...
												.row(
														$(
																...
																.parents(
																		...
																		.data();
												var path = SITEBASEURL
												+ 'community91OpsOrder/manifest-list/'
												+ data.sellerDTO.sellerId;
												$(
														"<form action='"
														+ path
														+ "'></form>")
...
{
		var statisticsUrl = SITEBASEURL + "community91OpsOrder/order-count-for-date?fromDate="+$("#fromDate").val() + "&toDate=" + $("#toDate").val();;
			var requestMethodType = "GET";
			$.ajax({
				url : statisticsUrl,
				type : requestMethodType,
...
function updateStatisticsForDate(result) {
	...
	$("#communityPlacedOrderforDate").html("For Date : "+ result.communityPlacedOrderforDate);
