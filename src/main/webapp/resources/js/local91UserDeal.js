...
function callToResult() {
    ...
    var reportUrl = SITEBASEURL + "local91UserDeal/listing?saleType="+$("#saleType").val();
    ...
    $("#groupBuyTable").on('draw.dt', function() {
    	$(".dt-edit").each(function() {
    		...
    		$(this).on('click', function() {
    			...
    			var path = SITEBASEURL + 'local91UserDeal/get/' + data.id;
    	...
    	$(".dt-remove").each(function() {
    		...
    		$(this).on('click', function() {
    			if (confirm('are you sure you want to delete this record?')) {
    				var table = $('#groupBuyTable').DataTable();
    				var data = table.row($(this).parents('tr')).data();
    				var path = SITEBASEURL + 'local91UserDeal/delete/' + data.id;
    				$("<form action='" + path + "'></form>").appendTo('body').submit();
    			}
    			else{
    				return false;
...
function callToSearchResults() {
    ...
    var reportUrl = SITEBASEURL + "local91UserDeal/listing";
