$(document).ready(function() {
	...
	$("#downloadReceipt").click(function() {
		if (dukaan91Ids != null && dukaan91Ids != '') {
			window.open(SITEBASEURL + 'dukan91OpsOrder/bulk-receipt/'+ dukaan91Ids, '_blank');
		} else {
...
function callToSearchResults() {
	var reportUrl = SITEBASEURL + "dukan91OpsOrder/pickList-pagelist?startDate=" +$("#fromDate").val() +"&endDate="+ $("#toDate").val()+"&searchTerm="+ $("#searchTerm").val()+"&courierPartner=" +$("#courierPartner").val();
											...
											$(this)
													...
													.on(
															...
															function() {
																...
																+ 'dukan91OpsOrder/manifest-receipt-by-identifier/'
																+ data.identifier;
																$(
																		"<form action='"
																				+ path
									...
									.on(
											...
											function() {
												...
												var data = table
														...
														.data();
												var path = SITEBASEURL
												+ 'dukan91OpsOrder/get-manifest-orders/' +data.id+'/'+$("#courierPartner").val();
									...
									.on(
											...
											function() {
												...
												var data = table
														...
														.data();
												var path = SITEBASEURL
												+ 'dukan91OpsOrder/manifest-invoice-by-identifier/'
												+ data.identifier;
												$(
														"<form action='"
																+ path
...
function addd91IdsToArray(id){
	...
	dukaan91Ids.push(id);
		...
		var index = dukaan91Ids.indexOf(id);
		if (index > -1) {
			dukaan91Ids.splice(index, 1);
		}
	}
