...
function callToSearchResults() {
	...
	var reportUrl = SITEBASEURL + "pickupLot/page-list-for-return-pickup-lot?searchTerm="+ $("#searchTerm").val();
			...
			.dataTable(
					{
						...
						"aoColumns" : [ {
						 ...
						 {
			            	   "sTitle" : "Mark Lot Received",
			            	   "bSortable" : false,
			            		'mRender': function( url, type, full ) {
			            			if(full.pickupLotStatus == "LOT_PICKED_UP")
			            							            				{
			            				return '<input type="text" class="form-control" style="height:0;position:absolute;z-index: -1;opacity: .01;" id="markLotReceived" /><button onclick="callToUpdateLotStatus(\''+full.identifier+'\',\''+full.pickupLotStatus+'\')" class="btn btn-custom">MARK_LOT_RECEIVED</button>';	  
			            				}
			            			 return "-";
			    						}
			               }
...
function callToManifestOrders(id)
{
	...
	if($("#searchTerm").val() == "" || $("#searchTerm").val() == null){
		window.location.href = SITEBASEURL+"pickupLot/get-manifest-orders/"+id+"?isReturnPickupLot=true&forAllPickupLots=true";
	}
	else{
	window.location.href = SITEBASEURL+"pickupLot/get-manifest-orders/"+id+"?isReturnPickupLot=true";
	}
}