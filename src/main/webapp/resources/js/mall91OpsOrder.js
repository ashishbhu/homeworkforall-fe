$(document).ready(
		function() {
			...
			callToSearchResults();
				...
				$("#searchByLmallOrderId").click(function() {
					if($("#lmallOrderId").val() == ''){
						alert("Enter Lmall orderId");
						return;
					}
...
var reportUrl = SITEBASEURL + "mall91OpsOrder/pagelist?orderState="
				...
				+"&stickerNumber=" +$("#stickerNumber").val()+"&itemId=" +$("#itemId").val()+"&lmallOrderId=" +$("#lmallOrderId").val();
			...
			.dataTable(
					{
						...
						},
						{
							...
							"render": function (referenceOrderId, type, row) {
						        if (row.referenceOrderId == null || row.referenceOrderId == undefined) {
						            	return "-";
						            }
						            else {
									...
									$(this).unbind().on(
											...
											function() {
												...
												if (selectedState != '0') {
													...
													if(( data.imei !=undefined && data.imei !== null && data.imei !== '') || (data.expiryDateUI !=undefined &&  
															data.expiryDateUI !== null && data.expiryDateUI !== '') ) {
														// do something
														...
														$('#expiryDateUI').prop('readonly', true);
														$(".modal-body #expiryDateUI")
													...
													else if(selectedState!='ORDER_READY_TO_DISPATCH')
													{
														...
														$('.modal-body #expiryDateUI').removeAttr('readonly');
														$('.modal-body #expiryDateUI').datetimepicker({
															format: 'DD-MM-YYYY'
														});
...
function callEdit(id){
	...
	window.location.href = SITEBASEURL + 'mall91OpsOrder/get/'+id+ '?orderSupplier='+$("#orderSupplier").val()+"&orderState="+$("#orderState").val();
