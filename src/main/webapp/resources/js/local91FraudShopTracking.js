...
function callToResults(){
	if ($("#shopIdFdt").val() != 'undefined' && $("#shopIdFdt").val() > 0) {
		 $("#txtSearch").attr("placeholder", "Enter Shop Id");
		 $("#txtSearch").val($("#shopIdFdt").val());
	}
	var reportUrl = SITEBASEURL + "local91Shop/fraud-detection-shop-listing?searchType="+ $("#searchType").val()+"&txtSearch="+ $("#txtSearch").val();
	...
	$("#groupBuyTable").on('draw.dt', function() {
        ...
        $(".dt-editmark").each(function() {
			...
			if(data.shopVerified != 'BLOCKED')
			{
                 	...
                 	var path = SITEBASEURL + 'local91Shop/update-shopstatus/' + data.id+"/"+ shopVerified + "?fraudDetectionPage=true";;
	    			$("<form action='" + path + "'></form>").appendTo('body').submit();
	    			}
...
function callToUpdateShopStatusFromModal(){
	...
	$("#loader").show();
	window.location.href = SITEBASEURL+'local91Shop/update-shopstatus/' + shopId+"/"+ status+"?fraudDetectionPage=true&comment="+comment ;
	}
...
function callToUpdateShopStatus(id,status){
	window.location.href = SITEBASEURL+"local91Shop/update-shopstatus/"+id+"/"+status+"?fraudDetectionPage=true";
}
