...
function callToResults(){
	var reportUrl = SITEBASEURL + "local91Product/category/item/attribute/dataList?category="+$("#rootCategoryId").val()+"&childCategoryId="+$("#categoryId").val();
	...
	$("#groupBuyTable").on('draw.dt', function() {
        $(".dt-edit").each(function() {
            ...
            $(this).on('click', function() {
                ...
                var path = SITEBASEURL + 'local91Product/get/category-item-attributes/' + data.id+"/"+$("#rootCategoryId").val()+"/"+$("#categoryId").val();
        ...
        $(".dt-remove").each(function() {
            ...
            $(this).on('click', function() {
                if (confirm('are you sure you want to Inactive this record?')) {
                    var table = $('#groupBuyTable').DataTable();
                    var data = table.row($(this).parents('tr')).data();
                    var path = SITEBASEURL + 'local91Product/inactive/categoryItemAttributes/' + data.itemAttributeId;
                    $("<form action='" + path + "'></form>").appendTo('body').submit();
                }
                else{
                    return false;
